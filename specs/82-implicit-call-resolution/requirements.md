...
| 8   | 未決事項解消       | 保留 | 2026-08-11 | 4 件を設計フェーズ (spec) 送りとして管理 |
| 9   | 最終レビュー       | 完了 | 2026-08-11 | ユーザー承認済み (draft 提示 → 起票承認) |
...
- Mapper 系マーカーの拡張 (`@FeignClient` / XML ベース MyBatis) — 既存の `@Mapper` / Spring Data 対応で据え置き
...
| R3  | イベント edge は「候補列挙」として扱い、条件付き listener 等で一意に絞れない場合は曖昧候補として出力する      | Spring DI の曖昧候補の扱いと一貫させる                      |                            |
...
| #   | 論点                                                                      | 決定者      | 期限        | 状態 | メモ                                                         |
| --- | ------------------------------------------------------------------------- | ----------- | ----------- | ---- | ------------------------------------------------------------ |
| 1   | callable 値渡しの静的追跡範囲 (同一メソッド内 / クラス内 / Bean 境界越え) | 設計 (spec) | scaffold 時 | 未決 | 範囲が広いほど精度は上がるが誤 edge リスクと計算量が増える   |
| 2   | entry point / イベント edge の Protocol (JSONL) 上の表現                  | 設計 (spec) | scaffold 時 | 未決 | 既存スキーマ拡張の後方互換。ADR 要否も設計で判断             |
| 3   | 対象アノテーション集合の確定 (合成アノテーションの検出深さ含む)           | 設計 (spec) | scaffold 時 | 未決 | Spring 標準に限るか、jakarta (`@PostConstruct`) の版差を含む |
| 4   | 実装の分割 (1 PR か、entry point / イベント / callable の 3 段階か)       | 設計 (spec) | prompts 時  | 未決 | issue は 1 本、実装 prompts は分割の想定                     |
...
- [x] 権限要件が確定している (該当なし)
- [x] 監査/非機能要件が確定している
- [ ] 未決事項がゼロ、または担当者・期限付きで管理されている (4 件を設計フェーズ送りとして管理中)
