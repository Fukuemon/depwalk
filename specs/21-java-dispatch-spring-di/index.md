...
| 5   | 論点解決                    | レビュー済 | 2026-07-15 | D1〜D9 すべて解決済み (D7: Lombok は SootUp bytecode 照会で解決、D8: MyBatis `@Mapper` を runtime-provided マーカーに追加、D9: Core の `callEdge.metadata` passthrough は #22 D11 へ委譲。`methodSymbol.metadata` は当初「両 issue の対象外」としたが、2026-07-15 に #22 側の D11 拡張がこれを override し #22 D11 の実装範囲に含めた、詳細は D9 override 追記参照)。マルチモジュール対応は #24 継続 |
...
| 10  | 実装分割                    | レビュー済 | 2026-07-14 | P1〜P3 に classpath/E3、Spring 選択、metadata、SootUp 座標、fixture build/classpath、Go E2E、同一 `feature/21` branch での直列実行を明記。fresh-context 最終再レビュー PASS                                                                                                                                                                                                                          |
| 11  | レビュー済                  | 完了       | 2026-07-14 | fresh-context 監査2回の NEEDS_WORKを全件修正し、最終再レビュー PASS。実装着手可                                                                                                                                                                                                                                                                                                                      |
...
- WHEN Spring Boot E2E fixture を解析したとき、既知の caller / callee 集合と一致する。#21 の受け入れ範囲は Analyzer JSONL と graph の照合までとし、#22 の完了を前提にしない。CLI 出力レベルの照合と metadata passthrough は #22 D11 の受け入れ範囲とする。
...
| D9  | D2/D6 が前提とする `callEdge.metadata` が Core の内部表現に到達する前に消失することが実装レビューで判明。`core/internal/graph/convert.go` の `EdgeFromCallEdge` が `Metadata` をコピーせず、`graph.Edge` に `Metadata` がない。#21 が修正すべきか | B: [Issue #22](https://github.com/Fukuemon/depwalk/issues/22) の branch-local spec D11 (2026-07-12 決定) が同じ `callEdge.metadata` gap を `graph.Edge`/`output.EdgeView` の透過追加として割当済みのため、#21 は重複実装せず委譲する。併せて発見した `methodSymbol.metadata` / `graph.Symbol` gap は #21 の解決根拠表現に使わず、#22 D11 も対象外のため、両 issue の実装範囲に含めない | 解決済み |
...
- **D2: 複数 dispatch 候補は「call site ごとに caller → 各実装候補への複数 CallEdge」で表す (案 A1)。宣言型 (interface / 基底型) への既存 edge も保持する。追加する実装候補 edge の metadata は `resolution` (`unique` / `ambiguous`)、`provenance` (重複なし・辞書順の `sootup` / `spring-di` 配列)、必要に応じて `conditional` / `conditionTypes` を持つ。**
  ...
  - 決定日: 2026-07-12
  - 決定者: Fukuemon
  - **追記 (2026-07-14、D9 決定に伴う訂正)**: 理由(3)の「Core / Traversal 変更ゼロ」は到達性 (edge の存在・BFS 到達) についてのみ正しい。metadata (`resolution` / `provenance`) 自体を Core が保持するための実装は [Issue #22](https://github.com/Fukuemon/depwalk/issues/22) D11 が別途担う (D9 参照)。D2 の edge 構造の決定自体は変更しない。
...
- **D6: 曖昧性・解決根拠の観測は、#21 では Analyzer JSONL の metadata + diagnostic までを責務とする (案 A)。CLI 出力 (Console / JSON) への edge 単位の metadata 表出は #22 (CLI interface spec) の論点として引き継ぐ。**
  ...
  - 決定日: 2026-07-12
  - 決定者: Fukuemon
  - **追記 (2026-07-14、D9 決定に伴う訂正)**: 「Analyzer JSONL の metadata までが #21 の責務」という切り分け自体は正しかったが、Core が metadata を内部保持しているという黙示の前提が誤りだったことが実装レビューで判明した (D9)。Core 側の opaque passthrough 実装は #21 ではなく [Issue #22](https://github.com/Fukuemon/depwalk/issues/22) D11 が担う。D6 の責務境界の決定自体 (JSONL までが #21、CLI 表出は #22) は変更しない。
...
- **D7: Lombok が生成するコンストラクタ (`@AllArgsConstructor` / `@RequiredArgsConstructor` 等) は、SootUp の bytecode 型階層照会対象に自プロジェクトのコンパイル済み class を含めることで解決する (案 B)。**
  ...
  - 入力契約: 自プロジェクトの classes output directory は既存 `analysisRequest.metadata.classpath` に追加する。依存 jar と同じ配列を再利用し、新しい metadata key は導入しない。明示した classpath entry が存在しない、または読めない場合は既存 pre-flight 契約どおり `JAVA_MISSING_JAR` の fatal とする。
  - fallback: classpath が pre-flight を通過した後に SootUp が class file を解釈・索引化できない場合、または自プロジェクトの classes directory が指定されず source に対応する bytecode を取得できない場合だけ、`JAVA_SOOTUP_UNAVAILABLE` を出して JavaParser 結果で継続する。
  - 決定日: 2026-07-14
  - 決定者: Fukuemon
...
- **D9: #21 が解決根拠を載せる `callEdge.metadata` の Core 内消失は #22 D11 へ委譲する。`methodSymbol.metadata` の既存 gap は委譲対象に含めない。**
  - 決定理由: `core/internal/graph/convert.go` の `EdgeFromCallEdge` が `protocol.CallEdge.Metadata` をコピーせず、`graph.Edge` に `Metadata` がない。一方、[Issue #22](https://github.com/Fukuemon/depwalk/issues/22) の branch-local spec D11 は、`graph.Edge` と `output.EdgeView` に Metadata を非破壊で追加し、JSON formatterへ透過する実装を先に確定している。#21 が同じ Core code を変更すると競合するため委譲する。
  - `NodeFromMethodSymbol` / `graph.Symbol` にも同種の gap はあるが、#21 の `resolution` / `provenance` はすべて `callEdge.metadata` に載せ、新しい `methodSymbol.metadata` の利用を受け入れ条件に含めない。#22 D11 も `graph.Symbol` を明示的に対象外としているため、これは #21/#22 D11 のどちらにも委譲しない。将来の method symbol metadata 利用 issue で設計する。
  - 対応方式: #21 は Core / Output の本番実装を行わない。#21 の Analyzer JSONL / graph E2E は #22 に依存せず完了でき、`callEdge.metadata` の Core 内保持と CLI JSON 表出だけが #22 D11 の実装完了に依存する。追跡先は branch-local path ではなく [Issue #22](https://github.com/Fukuemon/depwalk/issues/22) とする。
  ...
  - 決定日: 2026-07-14
  - 決定者: Fukuemon
  - **override (2026-07-15、#22 側の再検証)**: 上記「`methodSymbol.metadata` の既存 gap は委譲対象に含めない (両 issue 対象外、将来issue)」の部分は、[Issue #22](https://github.com/Fukuemon/depwalk/issues/22) の spec (`specs/22-cli-interface/index.md` D11 拡張) がこの切り分けを override し、`graph.Symbol`/`output.NodeView` への Metadata 透過も #22 D11 の実装範囲に含めることを決定した。理由: `methodSymbol.metadata.declaringType`/`inherited` は spec #9 D11 (帰属型の決定規則) 起源で Analyzer が実際に出力しており、Edge 側 (`callEdge.metadata`) だけを透過して Node 側を欠落させたままにすると CLI JSON 出力が非対称になる。D9 の他の部分 (callEdge.metadata の委譲、#21 は Core/Output の本番実装を行わない、という対応方式) は変更しない。決定者: Fukuemon (via #22 spec-lifecycle)。
...
| モジュール          | 実装有無 | 主な責務                                                                                                                                                                                                                    |
| ------------------- | :------: | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `core`              |    ◯     | `core/e2e` の test code のみ変更する。本番 code は変更しない。D9 の metadata passthrough 本番実装は [Issue #22](https://github.com/Fukuemon/depwalk/issues/22) の D11 が担う。Spring / JVM / SootUp の意味は解釈しない (S5) |
...
- `analysisRequest` / `methodSymbol` / `callEdge` / `diagnostic` の既存 schema を変更しない。D2 の metadata key/value と Spring Bean 名・Qualifier・Primary 選択規則は [feature doc](../../design/features/java-analyzer/DesignDoc_java-analyzer.md#dispatch-標識) を正本とする。SootUp 由来の型階層情報は edge の正本にはならず、JavaParser 側が生成する call edge の入力としてのみ使う (D1)。
...
| E3  | pre-flight 通過後に SootUp が class file を解釈・索引化できない、または自プロジェクト bytecode が classpath にない | `JAVA_SOOTUP_UNAVAILABLE` と対象・原因を diagnostic へ出力                                                                                                                                       | JavaParser 結果のみで解析継続。明示 classpath entry の欠落・読み取り不能は既存 `JAVA_MISSING_JAR` fatal のまま |
...
- Spring Boot fixture による E2E で、既知の caller / callee 集合との照合を行う。実行コマンドは `(cd analyzers/java && ./gradlew shadowJar) && analyzers/java/gradlew -p testdata/fixtures/java/spring-project clean classes writeDepwalkClasspath && (cd core && DEPWALK_E2E_REQUIRED=1 go test ./e2e -count=1)`。fixture task が classes directory と runtime jar の manifest を生成し、Go E2E が全 entry を `analysisRequest.metadata.classpath` に渡す。CLI 出力照合は #22 の受け入れ範囲とする。
...
> 以下は決定時スナップショット。正本は [Java Analyzer feature doc の Interface Dispatch / Spring DI 解決フロー](../../design/features/java-analyzer/DesignDoc_java-analyzer.md#interface-dispatch--spring-di-解決フロー)。本節は issue 単位の決定経緯として残す。
...
ADR-0005 の実装 prompt 順序 (型階層補完 → Spring 候補絞り込み → 統合 E2E) を踏襲する。tasks phase で `specs/21-java-dispatch-spring-di/prompts/` に生成済み。3 phase とも同一 `feature/21` branch と同一 PR 上で直列実行する。prompt ごとに同名 branch / PR を作り直さない。Core 側の metadata 保持本番実装 (D9) は #21 のスコープ外で、[Issue #22](https://github.com/Fukuemon/depwalk/issues/22) の D11 が担う。
...
| P3    | java-analyzer | 候補統合・CallEdge 化・重複排除 (D2)、diagnostic 拡張 (D6)、Spring Boot E2E fixture 新規作成、統合 E2E、性能計測・記録 (D5)                                          | P2   | `P3_01_java-analyzer_candidate-merge-fixture-e2e.md` |
...
| feature doc (analyzer-protocol) `methodSymbol`/`callEdge` の `metadata` フィールド説明 | #21 が利用する `callEdge.metadata` は #22 D11 で opaque passthrough として保持する。`methodSymbol.metadata` の既存 gap は当初 #21/#22 D11 の対象外としたが (D9)、2026-07-15 に #22 D11 拡張がこれを override し `graph.Symbol`/`output.NodeView` への opaque passthrough も対象に含めた (D9 override 追記参照) | source: clarify (spec D9 で決定、2026-07-15 override 分は #22 側 sync) | 反映済 (2026-07-15、override 分を feature doc へ再反映) |
...
| 2026-07-14 | NEEDS_WORK→修正済み      | fresh-context 実装前監査: E3/fatal 契約衝突、classpath classes directory 入力、metadata/Bean 選択規則、core E2E target、E2E command、メタ情報、正本ハンドオフの不足                                                                                   | 正本・P1/P2/P3・context を更新。再レビューへ                                            |
| 2026-07-14 | NEEDS_WORK→修正済み      | fresh-context 再監査: feature doc scope、requirements同期、Spring fixture build/classpath、#22 D11のmethodSymbol境界、SootUp座標/versionが不足                                                                                                        | 正本・requirements・P1/P3を更新。再レビューへ                                           |
| 2026-07-14 | PASS                     | fresh-context 最終再レビュー: 上位文書、requirements、実装対象、E3/Spring/metadata、prompts、#22境界、正本・状態同期に blocker なし                                                                                                                   | 設計完了・実装着手可として確定                                                          |
...
| 2026-07-14 | Claude | D9 の初期案として `callEdge.metadata`/`methodSymbol.metadata` の両方を #21 で opaque passthrough する案を反映。後続の #22 cross-check と fresh review で撤回・訂正済み (現在の決定は D9 本文と後続履歴を参照)                                                                                                                                                                                                                                                                       |
| 2026-07-14 | Claude | D9 追加分 spec-review PASS。フェーズ5 (論点解決)・10 (実装分割)・11 (レビュー済) をレビュー済/完了として最終化                                                                                                                                                                                                                                                                                                                                                                      |
| 2026-07-14 | Claude | #22 の branch-local spec を確認し、D11 (2026-07-12 決定) が D9 と同一の Core metadata 消失 gap を既に発見・実装スコープに割当済みであることが判明。D9 を「#21 が実装する (案 A)」から「#22 D11 へ委譲する (案 B)」に訂正。撤回内容: `prompts/P1_02_core_metadata-passthrough.md` を削除、P3 prompt の前提条件・E2E ステップから P1_02 依存を除去、実装対象表 `core`/`output` 行・実装タスク案テーブルを D9 追加前の状態に復元 (D6 の元の責務境界が正しかったことを確認する形の訂正) |
...
| 2026-07-15 | Claude | [Issue #22](https://github.com/Fukuemon/depwalk/issues/22) 側 (`feature/22` の develop rebase 後の再検証) が D9 の「`methodSymbol.metadata` は #21/#22 双方の対象外」を override し、#22 D11 拡張で `graph.Symbol`/`output.NodeView` への Metadata 透過も引き取ることを決定。D9 本文に override 追記を反映 (callEdge.metadata の委譲・#21 が本番実装を行わない、という他の決定内容は不変)                                                                                           |
| 2026-07-15 | Claude | #22 phase 6 (Interface/Routing 設計) の spec-review で `design/features/analyzer-protocol/DesignDoc_analyzer-protocol.md` が override 前の記述のまま残っていた矛盾が検出されたため、#22 側で feature doc を override 後の内容へ更新。本 spec の `:470` (feature doc 反映済み行) を実際の反映内容・日付 (2026-07-15) に再同期                                                                                                                                                        |
