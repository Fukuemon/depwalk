...
| 5   | 論点解決                    | レビュー済 | 2026-07-14 | D1〜D9 すべて解決済み (D7: Lombok は SootUp bytecode 照会で解決、D8: MyBatis `@Mapper` を runtime-provided マーカーに追加、D9: Core の `callEdge.metadata` passthrough は #22 D11 へ委譲し、`methodSymbol.metadata` は両 issue の対象外)。マルチモジュール対応は #24 継続                                        |
