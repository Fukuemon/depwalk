...
| cli / graph / output / traversal / java-analyzer 各 doc | `core/internal/<pkg>` への path 参照 (grep 実測 計 13 箇所) を再編後の `core/internal/{domain,app,platform}/<pkg>` へ機械的追随 **[子 issue で実施 (D4: 実態追随の doc 修正)]** — **java-analyzer doc 分は #35 で追随不要と実測確認 (2026-07-25): D8 で core の物理移動を撤回したため `core/internal/<pkg>` 参照は不変。内部 package 構成の記述は 2026-07-24 sync で反映済みで実装と一致** | D1 決定に伴う path 追随 (source: track) |
...
| 2026-07-25 | Claude (#35 実装)       | #35 P1_02 / P3_02 完了 (pipeline 新設・ArchUnit 3 ルール導入)。P2_02 は前提誤りのため実装なし: D7 の「7 クラスに漏れている `sootup.*` import」は自前 package `analysis.sootup` の import の誤認で、SootUp ライブラリは全履歴で `SootUpTypeHierarchyIndex` 1 ファイルに封じ込め済み (ArchUnit で機械確認) |
