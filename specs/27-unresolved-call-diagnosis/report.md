...
| P4_04 (前進解決) 後      | **143 (-59.1%)** |   **1,203 (-91.6%)** |
...
残存はいずれも「classfile / 静的根拠で解決・分類できない」もので、根拠のない型推測で false edge を作らない方針 (feature doc / ADR-0004) の保守側に位置する。ADR-0004 の再検討条件 (「未解決が支配的」「主要ユースケースの精度を満たせない」) への抵触判定は P5_01 のユーザー承認で確定する。
