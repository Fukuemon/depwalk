...
| context     | `context/architecture.md` Package Boundary / Runtime Boundary | 反映済 |
...
- `specs/8-analyzer-protocol/`: Protocol / SPI / Model schema と contract test 観点 / 期待ケースの設計は決定済み。Core 実装基盤は ADR-0002 へ handoff 済み
- `adr/0002-core-implementation-foundation.md`: Go / Go modules / Go 標準 command / 初期 package 構成の正本
- 関連 issue / ticket: [#11](https://github.com/Fukuemon/depwalk/issues/11)、[issue #12](https://github.com/Fukuemon/depwalk/issues/12)
...
- 決定理由と却下案を ADR に昇格する。
- `context/project.md`、`context/architecture.md`、`context/toolchain.md`、`context/testing.md`、`context/engineering.md` の更新方針を決める。
- Core 実装基盤の最小 scaffold として、`core/go.mod`、Cobra 依存、`core/cmd/depwalk/` の最小 root command、`core/internal/...` の空 package、`analyzers/java/`、`testdata/analyzer-protocol/`、`testdata/fixtures/` を作る。`core/go.mod` の module path は `context/project.md` の Core module path に従う。`core/internal/...` には `package` 宣言のみの stub `.go` を置き、Git が追跡しない空 directory には `.gitkeep` などの placeholder file を置く。
- `cd core && go test ./...`、`cd core && go vet ./...`、`cd core && go build ./cmd/depwalk`、`cd core && test -z "$(gofmt -l .)"`、`cd core && go mod tidy` 後の差分確認が通る最小状態を作る。
...
- 技術選定 ADR が作成されている。
- `context/project.md` / `context/architecture.md` / `context/toolchain.md` / `context/testing.md` / `context/engineering.md` が更新されている。
- `core/go.mod` が `context/project.md` の Core module path で作成され、Cobra 依存が追加されている。
- `core/cmd/depwalk/` に `main` と Cobra root command の最小実装が作成され、`cd core && go build ./cmd/depwalk` が成功する。
- `core/internal/{cli,analyze,protocol,analyzer,graph,traversal,output}` に `package` 宣言のみの stub `.go` が作成されている。
- `analyzers/java/`、`testdata/analyzer-protocol/`、`testdata/fixtures/` が作成され、空 directory は placeholder file で Git 追跡可能になっている。
- `cd core && go test ./...`、`cd core && go vet ./...`、`cd core && test -z "$(gofmt -l .)"` が成功し、`cd core && go mod tidy` 後に `go.mod` / `go.sum` の意図しない差分が出ない。
- 依存境界確認として、`cd core && go list -deps ./...` と `cd core && go list -f '{{.ImportPath}} {{.Imports}}' ./...` の結果に `github.com/Fukuemon/depwalk/analyzers`、`analyzers/`、Analyzer runtime library への直接 import が含まれないことを確認できる。
- flowchart と実装分割表で、Protocol parser / validator / contract test は issue #12、Traversal、Output、Java Analyzer、CLI interface は各後続 spec に分割されている。
...
| #   | 論点 | 決定候補 | 決定 |
| --- | ---- | -------- | ---- |
| -   | 解決待ちの論点なし | - | D1-D7 は「解決済みの論点」を参照する |
...
- D5: `analyzer-protocol` の Go 実装配置と Analyzer process 境界は、Core が Analyzer runtime / implementation に直接依存しないことを最優先に決めた。詳細な package boundary は [ADR-0002](../../adr/0002-core-implementation-foundation.md) と [context/architecture.md](../../context/architecture.md) を正本とする。
- D6: 初期 directory / package 構成は `core/` と `analyzers/` を top-level に分ける判断を採用した。詳細な directory tree と各 package の責務は [ADR-0002](../../adr/0002-core-implementation-foundation.md)、[context/architecture.md](../../context/architecture.md)、[context/project.md](../../context/project.md) を正本とする。
...
JSONL parser / validator の実装方針は [ADR-0002](../../adr/0002-core-implementation-foundation.md) と [context/testing.md](../../context/testing.md) を正本とし、本 spec では再定義しない。
Protocol / SPI / Model schema の正本は Analyzer Protocol feature doc と ADR-0001 とする。
Contract test の横断観点は [context/testing.md](../../context/testing.md) を正本とする。
spec #8 は issue #8 の決定経緯と期待ケースの記録であり、issue #12 は Go 側の parser / validator / contract test code と fixture file を実装する。
...
| ---------- | ---------------- | ------ | ---- | ------------ |
| Analyzer process の timeout / stderr 上限 / record size 上限 | CLI 固定値 / runtime config / 初期未設定のいずれかを、fixture E2E での timeout・stderr 量・record size 測定を見て決める | Fukuemon | CLI interface spec または runtime config spec 着手前 | CLI の exit code、エラー表示、runtime config、E2E の失敗条件に影響する |
| Runtime budget の具体値 | sample fixture の p95 / CI 実行時間上限 / large repository fixture 測定値のいずれを基準にするか、初回 E2E 測定後に決める | Fukuemon | Traversal / Output / Java Analyzer の初回 E2E 測定後 | 性能目標、CI 実行時間、large repository 対応方針に影響する |
| 開発ツール (`golangci-lint` / `govulncheck`) の version 固定方法 | CI workflow pin / tools manifest / 当面未導入のいずれかを、CI gate 設計時に決める | Fukuemon | CI gate 設計時 | 再現性、CI cache、開発者 setup 手順に影響する |
...
| モジュール          | 実装有無 | 主な責務 |
| ------------------- | :------: | -------- |
| `core`              |    ◯     | `core/go.mod`、Cobra root command、`core/internal/...` stub、scaffold validation を作る |
| `traversal`         |    ◯     | 空 package 境界を作る。探索 engine の実装は Traversal spec で扱う |
| `output`            |    ◯     | 空 package 境界を作る。Console / JSON / DOT / Mermaid 出力の実装は Output spec で扱う |
| `analyzer-protocol` |    ◯     | 空 package 境界と fixture directory を作る。Protocol 契約は feature doc / ADR-0001、contract test 横断観点は context/testing、Go 実装は issue #12 で扱う |
| `java-analyzer`     |    ◯     | `analyzers/java/` directory placeholder だけ作る。Java 固有実装は Java Analyzer spec で扱う |

spec #11 の主対象は `core` domain の scaffold であり、既存 domain (`traversal` / `output` / `analyzer-protocol` / `java-analyzer`) は空 package / directory placeholder として扱う。
scaffold では `core/cmd/depwalk` に Cobra root command の最小実装を置き、`core/internal/cli`、`core/internal/analyze`、`core/internal/protocol`、`core/internal/analyzer`、`core/internal/graph`、`core/internal/traversal`、`core/internal/output` に `package` 宣言のみの stub `.go` を置く。
...
4. 設計者は `context/project.md`、`context/architecture.md`、`context/toolchain.md`、`context/testing.md`、`context/engineering.md` を更新する。
...
- Protocol / SPI / Model schema は Analyzer Protocol feature doc と ADR-0001 を正本とする。
- Contract test の横断観点は `context/testing.md` を正本とし、spec #8 は issue #8 の決定経緯と期待ケースの記録として参照する。
- issue #12 は Go 側の parser / validator / contract test code と fixture file を実装する。
- 本 spec では package 境界と fixture directory だけを作る。
...
- Core の unit test は Graph / Traversal / Output / Protocol parser / validator の責務ごとに配置する。spec #11 では空 package / placeholder だけを作り、テスト実装は後続 spec で扱う。
- Protocol contract test は `analyzer-protocol` 境界に置き、Core と Analyzer 実装が同じ contract を参照できるようにする。横断観点は `context/testing.md`、Protocol schema は Analyzer Protocol feature doc / ADR-0001 を正本とし、Go 側の test code と fixture file は issue #12 で実装する。
...
- `analyzer-protocol` 実装配置は `core/internal/protocol` と `testdata/analyzer-protocol/` の境界だけを決める。Protocol 契約は Analyzer Protocol feature doc / ADR-0001、parser / validator / contract test 実装は issue #12 で扱う。
...
- spec #11 では、上記 directory と最小 compile 用 placeholder だけを作る。
- package 責務の正本は [context/architecture.md](../../context/architecture.md) とし、本節では実装範囲の境界だけを扱う。
- `cmd/depwalk` は Cobra root command を起動できる最小状態にするが、`depwalk analyze` の interface は実装しない。Cobra 依存が `go mod tidy` で除去されないよう、root command から実際に Cobra を import する。
- `core/internal/...` は `package` 宣言のみの stub `.go` を持つ compile 対象の空 package に留め、Protocol parser、Analyzer process 制御、graph、traversal、output のロジックは入れない。
- `analyzers/java/`、`testdata/analyzer-protocol/`、`testdata/fixtures/` は `.gitkeep` などの placeholder file で Git 追跡可能な directory placeholder に留め、Java Analyzer 実装と fixture 内容は後続 spec で扱う。
...
- scaffold validation では、`cd core && go list -deps ./...` と `cd core && go list -f '{{.ImportPath}} {{.Imports}}' ./...` の結果に、`github.com/Fukuemon/depwalk/analyzers`、`analyzers/`、Analyzer runtime library への直接 import が含まれないことを確認する。scaffold 段階ではこの確認は境界の初期状態を固定するための smoke check とし、実効的な CI gate は Analyzer / Protocol 実装が入った時点で [context/engineering.md](../../context/engineering.md) へ具体化する。
- Protocol strict validation、nil slice / nil map、Analyzer process contract の詳細検証は [context/testing.md](../../context/testing.md) と Analyzer Protocol feature doc / ADR-0001 を参照し、実装時の検証は issue #12 で扱う。
- `cd core && go build ./cmd/depwalk` は spec #11 scaffold 固有の Cobra root command 確認であり、Core 全体 build の標準 command は [context/project.md](../../context/project.md) の `cd core && go build ./...` とする。
...
flowchart TD
    ...
    scaffold --> issue12["issue #12<br/>Protocol parser / validator / contract test"]
    scaffold --> traversal["Traversal spec<br/>graph / caller / callee traversal"]
    scaffold --> output["Output spec<br/>Console / JSON / DOT / Mermaid"]
...
sequenceDiagram
    ...
    Implementer->>Implementer: go test / go vet / gofmt -l / go mod tidy 後の差分を確認
...
| P4 | scaffold | `core/go.mod`、Cobra 依存、`core/cmd/depwalk/` の最小 root command、`core/internal/...` stub、`analyzers/java/`、`testdata/analyzer-protocol/`、`testdata/fixtures/` を作る。空 directory には placeholder file を置く | P1-P3 |
| P5 | scaffold validation | `cd core && go test ./...`、`cd core && go vet ./...`、`cd core && go build ./cmd/depwalk`、`cd core && test -z "$(gofmt -l .)"`、`cd core && go mod tidy` 後の差分確認、`go list -deps` / `go list -f '{{.ImportPath}} {{.Imports}}'` による依存境界確認を通す | P4 |
...
| 2 | issue #12 | Analyzer Protocol の parser / validator / contract test を実装する |
| 3 | Traversal spec | graph / caller / callee traversal を実装する |
| 4 | Output spec | Console / JSON / DOT / Mermaid などの出力を実装する |
...
| 対象 doc / 節 | 変更内容 | 理由 |
| ------------- | -------- | ---- |
| `context/project.md` Repository Map / Quick Commands / 対象ドメイン / Naming Conventions | 反映済: 初期 directory 構成、Core module path、`core` domain、build / test / quality gate command を追記 | 実装着手前の未確定項目を解消するため |
...
| `context/testing.md` テスト runtime contract | 反映済: 採用 test framework と実行 command、contract test 配置を追記 | issue #12 の実装 prompt 生成に必要なため |
...
| 2026-06-27 | NEEDS_WORK | issue #12 参照のリンク不足、Spec8 との責務分界、依存境界 gate、scaffold 対象表と placeholder 方針の不足 | 対応済み。再 review 待ち |
| 2026-06-27 | NEEDS_WORK | quality gate 表現の不整合、未確定事項の選択肢不足、EARS 検証条件、正本重複の残り | 対応済み。再 review 待ち |
...
| 2026-06-27 | Codex | 再 review 指摘に対応し、issue #12 リンク、Spec8 との責務分界、scaffold 検証条件、placeholder 方針を整理 |
| 2026-06-27 | Codex | Protocol 契約設計は spec #8、parser / validator / contract test 実装は issue #12 として参照を整理 |
| 2026-06-27 | Codex | 再 review 指摘に対応し、quality gate、未確定事項の候補、EARS 検証条件、正本重複を整理 |
