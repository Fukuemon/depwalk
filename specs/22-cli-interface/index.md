...
| `context/testing.md` テスト runtime contract (L26)・E2E 2 層構造 (L16, L20)                             | 「E2E の具体 CLI 引数等は後続の CLI interface spec で確定する」を本 spec の確定内容 (D9/D12、flag 体系・golden 照合) へ更新し、CLI 出力照合の完成を反映。状態=未反映 (source: phase 9、2026-07-20) | E2E 2 層構造の完成宣言が本 spec の実装完了に依存するため                                |
| `context/testing.md` protocol contract 観点 (L87 の「Traversal と既存 Output schema は表出しないこと」) | #22 実装後の状態 (JSON 出力の新規 `metadata` フィールドは透過表出、意味解釈はしない) へ現状化。状態=未反映 (source: phase 9 レビュー補足、2026-07-20)                                              | D11 実装後は「既存 = #22 以前のフィールド」の読みでのみ真になり、誤読の余地が生じるため |
