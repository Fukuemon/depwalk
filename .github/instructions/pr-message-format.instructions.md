...
- 「概要」・「関連するissue」・「対応内容」・「チェックリスト」・「残作業」は原則として必須とし、「技術的な詳細」「テスト内容」「関連Issue」はプロジェクトの運用ルールに応じて必須化してもよい。
- 箇条書きには、できる限り「何を」「どこに」「なぜ」変更したかが分かる粒度で記述する。
- 残作業がある場合は、「残作業」のセクションに箇条書きで記載し、必ず対応するissueを作成すること。issueに関するフォーマットは(`issue-message-format.mdc`)に沿って作成すること。
