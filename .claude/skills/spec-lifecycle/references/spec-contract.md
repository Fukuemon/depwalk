...
**design 側は現在の設計だけを持つ**。過去にどうだったか・なぜ変えたか・どの issue で決めたかは書かない。書きたくなったらそれは ADR にすべき判断であり、design からは `(判断の正本は ADR-NNNN)` の形でその判断を説明している箇所に 1 行参照する。改訂の日付と順序は git history で追う。

| 情報                                 | 置き場                             |
| ------------------------------------ | ---------------------------------- |
| 選択肢を比較して決めた判断とその理由 | ADR                                |
| 改訂の日付・順序                     | git history                        |
| 作業単位の議論                       | issue / PR / spec                  |
| 現在の設計・契約・制約               | Design Doc / feature doc / context |

issue 番号を design 側へ書いてよい例外は **未解決の論点を指すとき**だけ (「この点は #NN で検討中」は現在の状態であり、解決したら記述ごと消える)。この規約は spec / requirements には適用しない。作業文書は経緯を持つのが役目である。
