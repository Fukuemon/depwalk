...
- `Prefix` などの基本フォーマットは Conventional Commits に準拠しつつ、`language` による言語指定や箇条書き本文など、このリポジトリ向けのガイドラインを追加しています。
...
- **1つのコミットには1つの関心ごとを含める**。複数の関心ごとが含まれる場合は、適切に分割して複数のコミットに分ける。
