---
name: spec-sync
description: Back-propagates spec decisions into PRD, Design Doc, feature doc, context, or ADR when the spec exceeds the current scope of those sources of truth. Use when "上位資料からの変更点" contains 変更提案 rows, or when spec-draft / spec-resolve flags upstream conflicts.
targets:
  - "*"
---
...
spec で確定した決定のうち、PRD / Design Doc / feature doc / context / ADR の範囲を超えるものを **上位文書に書き戻す** ための skill。
spec が `## 上位資料からの変更点` テーブルに残した「変更提案」行を処理する。

## いつ使うか

- spec の `## 上位資料からの変更点` に上位文書への変更提案が残っている
- `spec-draft` / `spec-resolve` が「変更提案」を理由に停止した
- 既存 spec の決定が PRD / Design Doc / feature doc / context / ADR と矛盾している
...
spec-resolve が決めた決定の中で、上位文書を書き換えるべきものを列挙する。
...
- spec 側の更新箇所
- 次アクション提案 (`spec-track` で差分仕上げ、または phase を先に進める)
