...
- 複数 context (project 依存関係) を要する Java unit test は、実 Gradle を起動せず in-memory の fake build model から production の context 構築を通して解析 pipeline を駆動する。model 取得 (Tooling API / daemon) の検証だけが E2E の責務 (境界の詳細は Java Analyzer feature doc のテスト観点)。
