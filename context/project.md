...
| やりたいこと                 | コマンド                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| ---------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 開発起動 (自動 discovery)    | `depwalk analyze <workspace-root> --language java --analyzer-cmd "java -jar analyzers/java/build/libs/java-analyzer.jar"` (`sourceRoots` / Java metadata を指定せず Gradle build model から discovery)                                                                                                                                                                                                                                                                    |
| 開発起動 (明示 override)     | `depwalk analyze <workspace-root> --language java --source-root <relative-root> --analyzer-cmd "java -jar analyzers/java/build/libs/java-analyzer.jar" --analyzer-meta classpath= --analyzer-meta javaLanguageLevel=21` (`--source-root` は繰り返し可。preview は `--analyzer-meta javaPreview=true`)                                                                                                                                                                     |
| 探索クエリ (caller 影響調査) | `depwalk analyze <workspace-root> --language java --analyzer-cmd "java -jar analyzers/java/build/libs/java-analyzer.jar" --method "com.example.UserService#findById(java.lang.Long)" --direction caller --format json` (`--method` は `<型の binary name>#<メソッド名>[(<引数型リスト>)]`。`--direction` 既定 `caller`、`--max-depth` 既定無制限、`--format` 既定 `console`。正本は [CLI feature doc](../design/features/cli/DesignDoc_cli.md)。設計確定済み・実装は #22) |
...
| Multi-project required E2E   | `(cd analyzers/java && ./gradlew shadowJar) && (cd core && DEPWALK_E2E_REQUIRED=1 go test ./e2e -run TestGradleMultiProjectCLI -count=1)` (実 Core CLI + 実 Analyzer jar。test-only 透過 proxy で request / raw graph / exit を照合)                                                                                                                                                                                                                                      |
| Gradle compatibility matrix  | `(cd analyzers/java && ./gradlew gradleCompatibilityTest)` (Gradle `7.6.5`〜`9.6.x` と daemon JVM の CI anchor を再現)                                                                                                                                                                                                                                                                                                                                                    |
