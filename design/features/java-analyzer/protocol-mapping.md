...
| `gradleJavaHome`          | string 配列 | 自動 discovery 時のみ任意 (要素 1 の path)。明示 `sourceRoots` 時は使用しない。要素数違反・非実在 / 実行不能な path は `JAVA_INVALID_REQUEST` で拒否 | 自動 discovery の Gradle daemon JVM を明示指定する。daemon JVM が対象 Gradle の互換範囲外になる場合の回避手段 (規則は [discovery.md](discovery.md)、判断の正本は [ADR-0012](../../../adr/0012-implicit-call-resolution-and-type-propagation-rescue.md)) |
