...
| `gradleJavaHome`          | string 配列 | 自動 discovery 時のみ任意 (要素 1 の path)。明示 `sourceRoots` 時は使用しない。要素数違反・非実在 / 実行不能な path は `JAVA_INVALID_REQUEST` で拒否。未実装 (実装は #82 で進行中) | 自動 discovery の Gradle daemon JVM を明示指定する。daemon JVM が対象 Gradle の互換範囲外になる場合の回避手段 (規則は [discovery.md](discovery.md)、判断の正本は [ADR-0012](../../../adr/0012-implicit-call-resolution-and-type-propagation-rescue.md)) |
...
| `JAVA_EVENT_UNRESOLVED`     | `warning` | `publishEvent()` の引数型が静的に解決できず、イベント edge を張れない (未実装、#82)                                                                                                       |
| `JAVA_CALLABLE_UNRESOLVED`  | `info`    | callable が静的追跡範囲 (同一メソッド内 / 引数渡し 1 段) の外にあり、invocation edge を張れない。設計上の制約による対象外を表す (未実装、#82)                                             |
