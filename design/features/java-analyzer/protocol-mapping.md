...
| `gradleJavaHome`          | string 配列 | 任意 (要素 1 の path)。未実装 (実装は #82 で進行中)                                     | 自動 discovery の Gradle daemon JVM を明示指定する。daemon JVM が対象 Gradle の互換範囲外になる場合の回避手段 (規則は [discovery.md](discovery.md)、判断の正本は [ADR-0012](../../../adr/0012-implicit-call-resolution-and-type-propagation-rescue.md)) |
