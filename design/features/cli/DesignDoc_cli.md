...
> 最終更新: 2026-07-20 / Status: 完了 ([issue #22](https://github.com/Fukuemon/depwalk/issues/22) の設計で新設。実装も同 issue が担う)
...
| 関連 issue     | [#22](https://github.com/Fukuemon/depwalk/issues/22)、[#24](https://github.com/Fukuemon/depwalk/issues/24) (`--source-root` / include・exclude の引き継ぎ元) |
...
探索クエリはサブコマンドを新設せず `analyze` コマンドへの flag 追加で提供する ([issue #22](https://github.com/Fukuemon/depwalk/issues/22))。
...
下表の flag はいずれも [issue #22](https://github.com/Fukuemon/depwalk/issues/22) で追加を決定した。
...
1 引数の統合書式 `<型の binary name>#<メソッド名>[(<引数型リスト>)]` とする ([issue #22](https://github.com/Fukuemon/depwalk/issues/22))。
...
- **analyze use case (`core/internal/analyze`)**: `AnalysisRequest` 組み立て (AnalysisMode は常時 `fullGraph` を明示、`Entrypoints` は空のまま。[issue #22](https://github.com/Fukuemon/depwalk/issues/22))、graph 構築後の method selector 照合・`traversal.Traverse`・`output.Write` の orchestration。照合の曖昧・不一致は候補一覧を含む種別付きエラーで CLI 層へ返す。
- 探索方向に関わらず常時 fullGraph で解析し、方向による挙動分岐を持たない ([issue #22](https://github.com/Fukuemon/depwalk/issues/22))。
...
- `os/exec` で build 済み depwalk バイナリを実プロセス起動し、stdout / stderr / exit code を検証する ([issue #22](https://github.com/Fukuemon/depwalk/issues/22)。harness は #24 整備の `buildCoreCLI`/`runCLI` を再利用)。
