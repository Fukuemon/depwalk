...
2. **1つのコミットに1つの関心ごとを含める**ことを確認する。複数の関心ごとが含まれる場合は、適切に分割して複数のコミットに分ける。
3. 変更のステージング（`git add -A` または特定ファイルのみ `git add <file>`）
...
- **1つのコミットには1つの関心ごとを含める**ことを原則とします。複数の関心ごとが含まれる場合は、適切に分割して複数のコミットに分けてください。
