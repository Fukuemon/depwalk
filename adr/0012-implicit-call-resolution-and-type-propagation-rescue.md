...
変更影響調査の網羅性は、edge が 1 本欠けると答えが間違う。そこに対し、framework が実行時に起動する暗黙の呼び出しが edge にならない欠落が残っていた。実環境検証プロジェクト (Gradle multi-project、7 project / call site 52,411) の実測で、次の 5 点が判明した。
