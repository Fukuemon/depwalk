...
- provider は Gradle `7.6.5` API に対してbuildしJava 8 classfileとする。対象Gradleは`7.6.5 <= version < 9.7.0`、wrapper不在時はbundled Tooling API `9.6.1`を使用する。固定CI anchorは`7.6.5 / daemon JDK 8`、`8.14.5 / daemon JDK 17`、`9.6.1 / daemon JDK 25`とし、詳細正本は [toolchain context](../context/toolchain.md#gradle-discovery-compatibility-matrix) とする。
